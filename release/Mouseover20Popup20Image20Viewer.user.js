...
You can edit the script's storage directly in your userscript manager.`);return}const t=setup.RULE||(setup.RULE=Symbol("rule"));let i,o=(elSetup||0).shadowRoot,{blankRuleElement:s}=setup,a,l=0,c=0,m=0,r=0;const n=setup.UI=new Proxy({},{get(f,y){return o.getElementById(y)}});elSetup||p(),elSetup.parentElement!==doc.body&&(g(await Config.load({save:!0})),doc.body.append(elSetup)),e&&S(e);function p(){elSetup=$new("div",{contentEditable:!0}),o=elSetup.attachShadow({mode:"open"}),o.append(...createSetupElement()),u()}function g(f){i=f,O(),M(),k(),requestAnimationFrame(()=>{n.css.style.minHeight=clamp(n.css.scrollHeight,40,elSetup.clientHeight/4)+"px"})}function u(){n._mover.onmousedown=h=>{h.button||eventModifiers(h)||(a||(a=n._cssSetup.sheet,a.insertRule(":host {}"),a=a.cssRules[1]),addEventListener("mousemove",y,!0),addEventListener("mouseup",w,!0),addEventListener("keydown",v,!0),l=h.x-m,c=h.y-r,$css(a,{opacity:.75}))},n._apply.onclick=n._cancel.onclick=n._ok.onclick=n._x.onclick=d,n._export.onclick=h=>{dropEvent(h),GM.setClipboard(Util.stringify(b(),null,"  ")),n._exportNotification.hidden=!1,setTimeout(()=>n._exportNotification.hidden=!0,1e3)},n._import.onclick=h=>{dropEvent(h);const A=prompt("Paste settings:");A&&g(new Config({data:A}))},n._install.onclick=setupRuleInstaller;const f=n._cssApp;n._reveal.onclick=h=>{h.preventDefault(),f.hidden=!f.hidden,f.hidden||(f.value||(App.updateStyles(),f.value=App.globalStyle.trim(),f.setSelectionRange(0,0)),f.focus())},n.start.onchange=function(){n[PREFIX+"setup"].dataset.start=this.value},n.xhr.onclick=({target:h})=>h.checked||confirm($propUp(h,"title"));for(const h of $$('[type="color"]',o))h.oninput=C,h.elSwatch=h.nextElementSibling,h.elOpacity=n[h.id.replace("Color","Opacity")],h.elOpacity.elColor=h;function y({x:h,y:A}){h=m=clamp(h-l,-innerWidth+CSS_SETUP_X*4,elSetup.clientWidth-CSS_SETUP_X),A=r=clamp(A-c,0,innerHeight-CSS_SETUP_X*3),$css(a,{transform:`translate(${h}px, ${A}px)`}),n._ul.style.maxHeight=`calc(100vh - ${CSS_SETUP_MAX_Y+A-CSS_SETUP_X}px)`}function w(){removeEventListener("mousemove",y,!0),removeEventListener("mouseup",w,!0),removeEventListener("keydown",v,!0),$css(a,{opacity:""})}function v(h){h.key==="Escape"&&!eventModifiers(h)&&(h.stopPropagation(),y({x:l,y:c}),w())}function C(){this.elSwatch.style.setProperty("--color",Util.color(this.value,this.elOpacity.valueAsNumber))}for(const h of $$('[type="range"]',o))h.oninput=L,h.onblur=z,h.addEventListener("focusin",T);function z(h){this.elEdit&&h.relatedTarget!==this.elEdit&&this.elEdit.onblur(h)}function T(){if(this.elEdit)return;const{min:h,max:A,step:q,value:N}=this;this.elEdit=$new("input",{value:N,min:h,max:A,step:q,className:"range-edit",style:`left: ${this.offsetLeft}px; margin-top: ${this.offsetHeight+1}px`,type:"number",elRange:this,onblur:I,oninput:R}),this.insertAdjacentElement("afterend",this.elEdit)}function L(){this.title=(this.dataset.title||"").replace("$",this.value),this.elColor&&this.elColor.oninput(),this.elEdit&&(this.elEdit.valueAsNumber=this.valueAsNumber)}function I(h){h.relatedTarget!==this.elRange&&(this.remove(),this.elRange.elEdit=null)}function R(){this.elRange.valueAsNumber=this.valueAsNumber,this.elRange.oninput()}o.addEventListener("keydown",h=>!h.altKey&&!h.metaKey&&h.stopPropagation(),!0)}function d(f){const y=this.id==="_apply";if(f&&(this.id==="_ok"||y)&&(cfg=i=b({save:!0,clone:y}),Ruler.init(),Menu.reRegisterAlt(),y))return M();$remove(elSetup),elSetup=null}function b({save:f,clone:y}={}){let w={};for(const v of $$("input[id], select[id]",o))w[v.id]=v.type==="checkbox"?v.checked:v.type==="number"||v.type==="range"?v.valueAsNumber:v.value||"";return Object.assign(w,{css:n.css.value.trim(),delay:n.delay.valueAsNumber*1e3,hosts:_(),scale:clamp(n.scale.valueAsNumber/100,0,1)+1,scales:n.scales.value.trim().split(/[,;]*\s+/).map(v=>v.replace(",",".")).filter(v=>!isNaN(parseFloat(v)))}),y&&(w=JSON.parse(Util.stringify(w))),new Config({data:w,save:f})}function _(){return[...n._rules.children].map(f=>[f.value.trim(),f[t]]).sort((f,y)=>f[0]<y[0]?-1:f[0]>y[0]?1:0).map(([f,y])=>y||f).filter(Boolean)}function E({target:f}){let y,w,v;const C=f.previousElementSibling;if(f.value){y=Ruler.parse(f.value),w=y instanceof Error&&(y.message||String(y));const z=!w&&y&&typeof y.d=="string"&&!/^[-.a-z0-9]*$/i.test(y.d);v=[z&&'Disabled due to invalid characters in "d"',w].filter(Boolean).join(`
`),f.classList.toggle("invalid-domain",z),f.classList.toggle("matching-domain",!!y.d&&hostname.includes(y.d)),C||f.insertAdjacentElement("beforebegin",s.cloneNode())}else C&&(C.focus(),f.remove());f[t]=!w&&y,f.title=v,f.setCustomValidity(w||"")}async function x({target:f,relatedTarget:y}){if(f!==this){if(await new Promise(setTimeout),f[t]&&f.rows<2){let w=f.selectionStart;const v=f.value=Ruler.format(f[t],{expand:!0});w+=v.slice(0,w).match(/^\s*/gm).reduce((C,z)=>C+z.length,0),f.setSelectionRange(w,w),f.rows=v.match(/^/gm).length}this.contains(y)||(y=[...$$('[style*="height"]',this)].find(w=>w!==f))}}function S(f){const y=n._rules.children;let w=[...y].find(C=>Util.deepEqual(C[t],f));if(!w){w=y[0],w[t]=f,w.value=Ruler.format(f),w.hidden=!1;const C=Math.max(0,_().indexOf(f));y[C].insertAdjacentElement("afterend",w),y[0].insertAdjacentElement("beforebegin",s.cloneNode())}const v=w.getBoundingClientRect();(v.bottom<0||v.bottom>w.parentNode.offsetHeight)&&w.scrollIntoView(),w.classList.add("highlight"),w.addEventListener("animationend",()=>w.classList.remove("highlight"),{once:!0}),w.focus()}function k(){const f=n._rules;f.addEventListener("input",E),f.addEventListener("focusin",x),f.addEventListener("paste",x),s=setup.blankRuleElement=setup.blankRuleElement||f.firstElementChild.cloneNode(),s.nextElementSibling&&(f.textContent="",f.appendChild(s.cloneNode()));for(const w of i.hosts||[]){const v=s.cloneNode();v.value=typeof w=="string"?w:Ruler.format(w),f.appendChild(v),E({target:v})}const y=n._search;y.oninput=()=>{setup.search=y.value;const w=y.value.toLowerCase();for(const v of f.children)v.hidden=w&&!v.value.toLowerCase().includes(w)},y.value=setup.search||"",y.value&&y.oninput()}function M(){n.scales.value=i.scales.join(" ").trim()||Config.DEFAULTS.scales.join(" "),n._css.textContent=cfg._getCss()}function O(){for(const f of $$("input[id], select[id], textarea[id]",o))f.id in i&&(f[f.type==="checkbox"?"checked":"value"]=i[f.id]);for(const f of $$('input[type="range"]',o))f.oninput();for(const f of $$('a[href^="http"]',o))Object.assign(f,{target:"_blank",rel:"noreferrer noopener external"});n.delay.valueAsNumber=i.delay/1e3,n.scale.valueAsNumber=Math.round(clamp(i.scale-1,0,1)*100),n.start.onchange()}}function setupClickedRule(e){let t;const i=e.target.closest("blockquote, code, pre");i&&!e.button&&!eventModifiers(e)&&(t=Ruler.fromElement(i))&&(dropEvent(e),setup({rule:t}))}async function setupRuleInstaller(e){dropEvent(e);const t=setup.UI._rules2;this.disabled=!0,this.textContent="Loading...";let i;try{i=o(await Req.getDoc(this.parentElement.href)),this.textContent="Rules loaded.";const m=$new("select",{size:8,style:"width: 100%",ondblclick:n=>n.target!==m&&c(n),onkeyup:n=>n.key==="Enter"&&c(n)},i.map(a)),r=m.selectedIndex=s();t.append($new("div#_installHint",["Double-click the rule (or select and press Enter) to add it. ","Click ",$new("code","Apply")," or ",$new("code","OK")," to confirm."]),m),r&&requestAnimationFrame(()=>{const n=m.selectedOptions[0].offsetTop-m.offsetTop;m.scrollTo(0,n-m.offsetHeight/2)}),m.focus()}catch(m){t.textContent="Error loading rules: "+(m.message||m)}function o({doc:m}){return[...$$("#wiki-body tr",m)].map(r=>[r.cells[0].textContent.trim(),Ruler.fromElement(r.cells[1])]).filter(([r,n])=>r&&n&&(!n.d||hostname.includes(n.d))).sort(([r],[n])=>(r=r.toLowerCase())<(n=n.toLowerCase())?-1:r>n?1:0)}function s(){const m=`.${hostname}.`;let r=0,n=0,p=0;for(const[g,{d:u}]of i){let d=!!(u&&hostname.includes(u))*10;for(const b of g.toLowerCase().split(/[^a-z\d.-]+/i))d+=m.includes(`.${b}.`)&&b.length;d>r&&(r=d,n=p),p++}return n}function a([m,r]){return $new("option",{textContent:m,title:Ruler.format(r,{expand:!0}).replace(/^{|\s*}$/g,"").split(`
`).slice(0,12).map(l).filter(Boolean).join(`
`)})}function l(m,r,n){return r===9&&n.length>10?"...":r>10?"":(m.length>100?m.slice(0,100)+"...":m).replace(/^\s/,"")}function c(m){eventModifiers(m)||setup({rule:i[m.currentTarget.selectedIndex][1]})}}const CSS_SETUP_X=20,CSS_SETUP_MAX_Y=200,CSS_SETUP=`
  :host {
